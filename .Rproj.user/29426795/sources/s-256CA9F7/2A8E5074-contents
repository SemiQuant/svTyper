    ...
    for sample in fileSets:
        ...
        for query in args.queries:
            ...
            if args.runtype == "improvement":
                ...
                run_command(['samtools', 'sort', five_to_ref_bam, " -o ", five_bam_sorted], shell=True)
                run_command(['samtools', 'sort', three_to_ref_bam, " -o ", three_bam_sorted], shell=True)
            ...
            if args.runtype == "typing":
                ...
                five_to_ref_bam = temp_folder + five_header + '_' + query_name + '.bam'
                three_to_ref_bam = temp_folder + three_header + '_' + query_name + '.bam'
                five_bam_sorted = five_header + '_' + query_name + '.sorted.bam'
                three_bam_sorted = three_header + '_' + query_name + '.sorted.bam'
                five_cov_bed = temp_folder + five_header + '_' + query_name + '_cov.bed'
                three_cov_bed = temp_folder + three_header + '_' + query_name + '_cov.bed'
            ...
            if args.bam == False:
                run_command(['rm', five_bam_sorted, three_bam_sorted, five_bam_sorted + '.bai', three_bam_sorted + '.bai'], shell=True)
